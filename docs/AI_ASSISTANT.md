...
Schemer uses the same persistent volume but accepts only `/workspace-schemer` sessions. Schemii history, message, activity, proposal, operation-status, and deletion routes resolve authority from the application-owned chat record; repeated browser context does not establish authority. Existing prefixed Schemii sessions are imported only after their legacy title binding is validated. Schemer retains its separate dashboard binding. Provider authentication remains shared because OpenCode stores it globally rather than per workspace.
