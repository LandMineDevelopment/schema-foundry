...
class SchemerAiExecutor:
    ...
    def execute(self, action, operation_id, *, chat, record, profile, schema_concurrency, authorization_target):
        ...
        if action_type == "read_query":
            ...
            result = self.service.execute_read_only_sql(profile["id"], authorization_target["namespace"], action.get("sql"), database=profile["dbname"], expected_profile_fingerprint=self.service.profile_context_fingerprint(profile["id"]), allow_explain=False, max_rows=100, max_columns=50, max_result_bytes=256 * 1024)
            reference = self.authority.create_result(chat["id"], {"resource": chat["dashboardId"], "target": authorization_target, "revision": record["revision"], "access": "data"}, bounded_ai_query_result(result, max_rows=100, max_columns=50, max_bytes=48 * 1024))
