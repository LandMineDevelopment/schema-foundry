...
class PostgresCatalogMixin:
    ...
    def _inspect_relation_connection(
        ...
        if descriptor["kind"] in {"view", "materialized_view"}:
            descriptor["dependencies"] = self._relation_lineage(
                connection, current, relation_row["live_oid"], dependents=False
            )
            descriptor["dependents"] = self._relation_lineage(
                connection, current, relation_row["live_oid"], dependents=True
            )
        else:
    ...
    def _relation_lineage(
        self, connection: Any, database: str, live_oid: int, *, dependents: bool
    ) -> dict[str, Any]:
        if dependents:
        ...
        rows = self._execute_rows(connection, f"""
            ...
            ORDER BY namespace, relation_name, relation_kind
            LIMIT %s
        ...
        return {
            ...
            "items": [
                ...
                for row in rows[:MAX_RELATION_LINEAGE_ITEMS]
            ],
            "truncated": len(rows) > MAX_RELATION_LINEAGE_ITEMS,
        }
