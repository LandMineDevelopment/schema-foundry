...
assert.match(source, /function canInspectViewsRelation[\s\S]*"partitioned_table"[\s\S]*"foreign_table"[\s\S]*if \(!source \|\| !canInspectViewsRelation\(source\)\) return;/, "all supported PostgreSQL relation kinds must be inspectable from verified lineage");
