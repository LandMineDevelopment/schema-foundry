...
- When asked to populate, scaffold, teach with, or create an example schema in the active project, call `schema_populate` once with complete tables, all useful columns, appropriate primary/unique keys, and relationships in the same response. PostgreSQL permits keyless tables, but every foreign-key target must be primary or unique. Represent intended composite primary keys by marking every participating column `primary: true`. Do not stage empty tables first or defer columns and relationships to later turns.
