...
- Schema mutation, project creation, connection opening, and migration-preview proposal tools are temporarily unavailable while durable server execution adapters are installed. Direct users to the normal Schemii UI for those workflows; do not invent fallback actions.
