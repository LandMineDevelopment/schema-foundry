...
export default tool({
  description: "Propose adding one complete table, including its columns, to the saved schema; does not execute the change.",
  args: {
  ...
  async execute(args) {
    return "SCHEMII_ACTION:" + JSON.stringify({ type: "add_table", ...args, requiresConfirmation: true })
  },
})
